...
<template>
  <div
    class="profile-card bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition"
  >
    ...
    <div class="flex items-start justify-between mb-4">
      ...
      <div class="text-right text-sm text-gray-600 ml-4">
        ...
        <span v-if="profile.job_title && (profile.company || profile.country)">
          •
        </span>
        ...
        <span class="font-semibold text-gray-700"
          >{{ Math.round(profile.similarity_score * 100) }}% Match Score</span
        >
      </div>
    </div>
    ...
    <div
      v-if="showMatchText && profile.text"
      class="mb-4 p-3 bg-blue-50 rounded border-l-4 border-blue-600"
    >
    ...
    <div
      ...
      class="mb-4 text-sm text-gray-600 bg-gray-50 p-3 rounded"
    >
      {{ profile.biography }}
    </div>
